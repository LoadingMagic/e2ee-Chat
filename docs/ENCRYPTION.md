...
RSA can only encrypt approximately 190 bytes with 2048-bit keys. Messages can be much longer. So we:
...
Each member can decrypt the group key with their private key, then use it to encrypt and decrypt all group messages.
